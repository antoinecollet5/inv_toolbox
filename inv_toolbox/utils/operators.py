...
def get_super_ilu_preconditioner(
    ...
    Parameters
    ----------
    mat : Union[csc_array, csc_matrix]
        The sparse matrix (in CSC format) to factorize.
    **kwargs
        Additional keyword arguments forwarded to
        :func:`scipy.sparse.linalg.spilu` (e.g. `drop_tol`, `fill_factor`).

    Returns
    -------
    Tuple[SuperLU, LinearOperator]
        A tuple ``(op, precond)`` where `op` is the :class:`SuperLU` factorization
        object and `precond` is a :class:`LinearOperator` applying the
        preconditioner (i.e. solving ``M x = b``) to be passed to an iterative
...
def get_angle_btw_vectors_rad(v1: NDArrayFloat, v2: NDArrayFloat) -> float:
    ...
    ----------
    v1 : NDArrayFloat
        First vector. Must be non-zero.
    v2 : NDArrayFloat
        Second vector. Must be non-zero.

    Returns
    -------
    float
        The angle between `v1` and `v2`, in radians, in ``[0, pi]``.

    Note
    ----
    The cosine of the angle is clipped to ``[-1, 1]`` before calling
    `arccos`, to guard against values that would otherwise fall (very)
    slightly outside that range due to floating-point round-off.
    """
    return np.arccos(
        np.clip(
...
def get_angle_btw_vectors_deg(v1: NDArrayFloat, v2: NDArrayFloat) -> float:
    ...
    -------
    float
        The angle between `v1` and `v2`, in degrees, in ``[0, 180]``.
    """
