...
class GaussianFilter(Filter):
    ...
    def filter(self, param: NDArrayFloat, iteration: int) -> NDArrayFloat:
        ...
        ------
        ValueError
            If `iteration` is lower than 1 (iterations start at 1, not 0;
            passing 0 or a negative value would otherwise silently wrap
            around to a sigma at the end of the `sigmas` sequence via
            negative indexing).
        """
        if iteration < 1:
            raise ValueError(
                f"iteration should be >= 1 (iterations start at 1), got {iteration}!"
            )
...
def get_sigma(
    ...
    ----------
    sigmas : Union[float, Sequence[Union[float, Sequence[float]]]]
        Either a single sigma value (applied at every iteration), or a
        sequence with one entry per iteration; each entry is itself either
        a single float (applied to every axis) or a per-axis sequence of
        floats (whose length must equal `dim`).
    index : int
        Index in the sequence of sigmas (i.e. ``iteration - 1``). If out of
        range, 0.0 is returned (no smoothing) rather than raising.
    dim : int
