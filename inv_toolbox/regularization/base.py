...
"""
Provide the base (abstract) classes for regularization and regularization
weight update strategies, as well as helper functions to build the sparse
spatial gradient and permutation matrices used by several regularizators.
...
class ConstantRegWeight(RegWeightUpdateStrategy):
    ...
    def __init__(self, reg_weight: float = 1.0) -> None:
        ...
        reg_weight : float, optional
            The (constant) regularization weight, by default 1.0.
        """
...
class Regularizator(ABC):
    ...
    def __init__(self, preconditioner: Preconditioner = NoTransform()) -> None:
        ...
        preconditioner : Preconditioner
            Parameter pre-transformation operator (variable change for the
            solver) applied before the regularization is evaluated. By
            default :class:`NoTransform`, which means no change is made.
        """
    ...
    def _eval_loss(self, values: NDArrayFloat) -> float:  # pragma: no cover
        """
        Compute the regularization loss function (implementation).

        Parameters
        ----------
        values : NDArrayFloat
            The (preconditioned) parameter for which the regularization is
            computed. Always a 1D vector.
...
def make_spatial_gradient_matrices(
    ...
    which : Literal["forward", "backward", "both"], optional
        Which finite-difference scheme(s) to accumulate into the returned
        matrices. "forward" only adds the forward-difference contribution,
        "backward" only the backward-difference contribution, and "both"
        adds both (their sum approximates a second-order/central scheme).
        By default "both".
