...
class TVRegularizator(Regularizator):
    ...
    Notes
    -----
    A spatial direction only contributes to the loss and gradient if the
    grid has strictly more than 2 cells along that direction (`grid.nx > 2`,
    resp. `grid.ny > 2`); a direction with 2 or fewer cells is silently
    ignored.
    ...
    def _get_grid_cell_l1(self, values: NDArrayFloat) -> NDArrayFloat:
        """
        Return the (regularized) local total-variation magnitude per cell.

        Parameters
        ----------
        values : NDArrayFloat
            2D array (nx, ny) of the field for which the local TV magnitude
            is computed.

        Returns
        -------
        NDArrayFloat
            Array of the same shape as `values`, containing
            :math:`\\sqrt{|\\nabla u|^2 + \\varepsilon}` at each cell.
        """
    ...
    def _eval_loss(self, values: NDArrayFloat) -> float:
        r"""
        Compute the Total Variation regularization loss function.

        .. math::

        \mathcal{R}_{TV}(u) = \sum_{j=1}^{M} \sum_{i=1}^{N} \sqrt{
        \left( \dfrac{u_{i+1, j} - u_{i,j}}{dx} \right)^{2}
        + \left( \dfrac{u_{i, j+1} - u_{i,j}}{dy} \right)^{2}
        + \varepsilon}
...
class TVMatRegularizator(Regularizator):
    ...
    def _get_grid_cell_l1(self, values: NDArrayFloat) -> NDArrayFloat:
        ...
        ----------
        values : NDArrayFloat
            1D (raveled, Fortran order) vector of the field for which the
            local TV magnitude is computed.

        Returns
        -------
        NDArrayFloat
            Vector of the same shape as `values`, containing
            :math:`\\sqrt{|\\nabla u|^2 + \\varepsilon}` at each cell.
        """
        # Add epsilon to prevent undetermination when deriving
        return np.sqrt(
    ...
    def _eval_loss(self, values: NDArrayFloat) -> float:
        r"""
        Compute the Total Variation regularization loss function (matrix formulation).
...
class TVFVMRegularizator(Regularizator):
    r"""
    Apply a Total Variation (sharpening) regularization using the Finite Volume Method.
    ...
    def _get_grid_cell_l1(self, v: NDArrayFloat) -> NDArrayFloat:
        ...
        Computed via the Finite Volume Method using the spatial permutation
        (adjacency) matrices, rather than a direct finite-difference stencil.
        ...
        -------
        NDArrayFloat
            Vector of the same shape as `v`, containing
            :math:`\\sqrt{|\\nabla u|^2 + \\varepsilon}` at each cell.
        """
        # Add epsilon to prevent undetermination when deriving
        arr = np.zeros_like(v)
    ...
    def _eval_loss(self, values: NDArrayFloat) -> float:
        r"""
        Compute the Total Variation regularization loss (Finite Volume Method).
