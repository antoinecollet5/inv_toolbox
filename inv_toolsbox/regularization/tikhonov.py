...
class TikhonovRegularizator(Regularizator):
    r"""
    Apply a Tikhonov (smoothing) regularization.
    ...
    Notes
    -----
    This implementation relies on a central finite-difference approximation
    of the Hessian. Consequently, a spatial direction is only included in
    the loss and gradient computation if the grid has strictly more than 2
    cells along that direction (`grid.nx > 2`, resp. `grid.ny > 2`); a
    ...
    def _eval_loss(self, values: NDArrayFloat) -> float:
        r"""
        Compute the Tikhonov regularization loss function.
...
class TikhonovMatRegularizator(Regularizator):
    r"""
    Apply a Tikhonov (smoothing) regularization using matrix formulation.
...
class TikhonovFVMRegularizator(Regularizator):
    r"""
    Apply a Tikhonov (smoothing) regularization using the Finite Volume Method.
    ...
    def _eval_loss(self, values: NDArrayFloat) -> float:
        r"""
        Compute the Tikhonov (Finite Volume Method) regularization loss function.
