...
"""
Implement adaptive regularization-weight update strategies.

Two families of strategies are provided: strategies that balance the norms
of the data-fit and regularization gradients (:class:`AdaptiveGradientNormRegweight`),
and strategies based on the U-curve (:class:`AdaptiveUCRegweight`).
...
class AdaptiveRegweight(RegWeightUpdateStrategy, ABC):
    ...
    is_use_first_adjusted_weight_as_upper_bound: bool
        ...
        upper bound. It prevents weight explosion.
    """
    ...
    def __init__(
        ...
        ------
        ValueError
            If `reg_weight_init` is negative, if the resulting bounds contain
            a negative value, if `max_log_cr` is not positive, or if
            `max_log_cr` is lower than or equal to `convergence_factor`.
        """
    ...
    def get_log_cr(_old: float, _new: float) -> float:
        """
        Return the logarithmic change rate between an old and a new value.

        Parameters
        ----------
        _old : float
            Old (previous) value. Always positive or null.
        _new : float
            New (current) value. Always positive or null.

        Returns
        -------
        float
            ``log(_new / _old)``, or 0.0 if either value is null (since the
            log-change-rate is then undefined).
        """
        if _old == 0 or _new == 0:
            return 0.0
    ...
    def _ensure_max_log_rc(
        ...
        gls_log_cr: Optional[float]
            Logarithmic change rate of the LS gradient norm (see
            :meth:`get_log_cr`), or None if not available.
        ...
        -------
        float
            The (possibly clipped) new regularization weight, such that its
            logarithmic change rate w.r.t. `old_rw` does not exceed the
            allowed maximum.
        """
        # maximum relative change
        if gls_log_cr is not None and gls_log_cr != 0:
...
class AdaptiveGradientNormRegweight(AdaptiveRegweight):
    ...
    def _update_reg_weight(
        ...
        """
        Update the regularization weight based on gradient norm balancing.
        ...
        loss_reg_grad : NDArrayFloat
            Current Reg cost function gradient.
        n_obs : int
            Number of observations used in the LS cost function. Currently
            unused by this strategy; kept for interface compatibility with
            :meth:`RegWeightUpdateStrategy.update_reg_weight`.
        logger: Optional[logging.Logger]
            Optional :class:`logging.Logger` instance used for event logging.
            The default is None.
        ...
        if self.is_noise_dominated:
            # The data-fit gradient is dominated by noise: increase the
            # regularization weight so that weight * ||grad_reg|| balances
            # ||grad_ls|| (this removes noise and prevents overfit).
            self.reg_weight = (
                (self.loss_reg_grad_norms[-1]) / self.loss_ls_grad_norms[-1]
            )
        else:
            # The data-fit gradient dominates the regularization gradient:
            # set the regularization weight so that weight * ||grad_reg||
            # balances ||grad_ls||.
            self.reg_weight = (
                self.loss_ls_grad_norms[-1] / (self.loss_reg_grad_norms[-1])
            )
...
class AdaptiveUCRegweight(AdaptiveRegweight):
    ...
    def _update_reg_weight(
        ...
        """
        Update the regularization weight based on the U-curve strategy.
        ...
        if self.n_regw_update < self.n_update_explo_phase:
            ...
            if self.is_noise_dominated:
                # The data-fit gradient is dominated by noise: increase the
                # regularization weight so that weight * ||grad_reg||
                # balances ||grad_ls|| (removes noise, prevents overfit).
                self.reg_weight = (
                    (self.loss_reg_grad_norms[-1]) / self.loss_ls_grad_norms[-1]
                )
            else:
                # The data-fit gradient dominates the regularization
                # gradient: set the regularization weight so that
                # weight * ||grad_reg|| balances ||grad_ls||.
                self.reg_weight = (
                    self.loss_ls_grad_norms[-1] / (self.loss_reg_grad_norms[-1])
                )
        ...
        # Case 2: we compute the optimal regularization weight - Optimization phase
            ...
            if not (
                ...
                raise ValueError(
                    "'loss_ls_history', 'loss_reg_history' and "
                    "'reg_weight_history' must have the same length! Got "
                    f"{len(loss_ls_history)}, {len(loss_reg_history)} and "
                    f"{len(reg_weight_history)} respectively."
                )
