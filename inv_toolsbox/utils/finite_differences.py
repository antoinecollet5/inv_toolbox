...
r"""
Provide finite difference approximation for the gradient and hessian.
...
def is_jacobian_correct(
    ...
    eps: float, optional
        The epsilon for the computation (h). If None (the default), it is
        computed as ``eps_machine ** (1/3) * min(1, max(abs(x)))`` (falling
        back to ``eps_machine ** (1/3)`` alone if `x` is all zeros), which
        should correspond to a near-optimal h taking into account the
        roundoff errors due to the machine precision.
    max_workers: int
...
def is_all_close(v1: np.ndarray, v2: np.ndarray, eps: float = 1e-2) -> bool:
    """
    Return whether the two arrays are approximately equal.

    Two entries ``v1[i]`` and ``v2[i]`` are considered close if
    ``|v1[i] - v2[i]| <= eps * max(|v1[i]|, |v2[i]|, 1.0)``, i.e. `eps` is a
    relative tolerance that switches to an absolute tolerance of `eps` near
    zero (to avoid an overly strict comparison there).
    ...
    v2 : np.ndarray
        Second array to compare. Must be broadcastable with `v1`.
    eps : float, optional
        Relative (and, near zero, absolute) tolerance. The default is 1e-2.

    Returns
    -------
    bool
        True if all entries of `v1` and `v2` are close, False otherwise.
    """
...
class FDParams:
    """
    Parameters shared by every per-element call of :func:`approximate_elt_jac`.

    Bundled into a single (picklable) dataclass so it can be passed as-is to
    worker processes when `finite_jacobian` runs with `max_workers` > 1.

    Attributes
    ----------
    x0 : NDArrayFloat
        Flattened (1D) point at which the Jacobian is evaluated. Perturbed
        and restored in place by :func:`approximate_elt_jac`.
    shape : Tuple[int]
        Original (unflattened) shape of `x0`, used to reshape it before
        calling `fm`.
    inner_steps : int
        Number of forward-model evaluations needed per element, i.e.
        ``2 * (accuracy + 1)``.
    coeff : List[List[float]]
        Per-accuracy-level list of finite-difference coefficients.
    coeff2 : List[List[float]]
        Per-accuracy-level list of step multipliers (in units of `eps`)
        at which `fm` is evaluated.
    fm : Callable
        Forward model.
    ...
    fm_kwargs : Dict[str, Any]
        Keyword arguments for the forward model.
    dd_val : float
        Normalization denominator (``dd[accuracy] * eps``) applied after
        summing the weighted forward-model evaluations.
    accuracy : int
        Selected accuracy level (0, 1, 2 or 3), indexing into `coeff`/`coeff2`.
    eps : float
...
def approximate_elt_jac(elt_id: int, fd_params: FDParams) -> NDArrayFloat:
    """
    Approximate the Jacobian column for a single input element by finite differences.

    Perturbs ``fd_params.x0[elt_id]`` in turn to each of the offsets given by
    ``fd_params.coeff2[fd_params.accuracy]`` (restoring it afterwards), evaluates
    the forward model at each perturbed point, and combines the results with
    ``fd_params.coeff[fd_params.accuracy]`` and ``fd_params.dd_val`` to form the
    finite-difference approximation of ``d(fm)/d(x0[elt_id])``.

    Parameters
    ----------
    elt_id : int
        Index (into the flattened `fd_params.x0`) of the input element to
        differentiate with respect to.
    fd_params : FDParams
        Shared finite-difference parameters (forward model, step size,
        coefficients, etc.).

    Returns
    -------
    NDArrayFloat
        The approximated derivative of the forward model's output with
        respect to ``x0[elt_id]``, with the forward model's output shape.
    """
    _res = []
...
def finite_jacobian(
    ...
    eps: float, optional
        The epsilon for the computation (h). If None (the default), it is
        computed from the machine precision as detailed below.
    ...
    ------
    ValueError
        If `accuracy` is not one of 0, 1, 2 or 3.
